...
$show_stat  = (bool) get_field('show_stat');

// ✅ (Fix) these were used but not defined
$stat_number = get_field('stat_number');
$stat_label  = get_field('stat_label');

// ✅ helper for responsive img data
function mz_img_payload($img, $size = 'large') {
  ...
  // circle image container: 200px, so we can guide browser
...
<section class="mz-w-full">
  <div class="mz-max-w-[1290px] mz-mx-auto mz-px-4 mz-py-10 md:mz-py-20 xl:mz-px-0">
    ...
    <?php if ($show_stat && ($stat_number || $stat_label)): ?>
    ...
    <div class="mz-grid mz-grid-cols-1 md:mz-grid-cols-2 xl:mz-grid-cols-4 mz-gap-y-14 mz-gap-x-10">
      ...
      <?php foreach ($steps as $i => $step): ?>
        <?php
          $imgp = mz_img_payload($step['image'], 'large');
          $alt  = $imgp && !empty($imgp['alt']) ? $imgp['alt'] : ('Step ' . ($i + 1) . ' - ' . ($step['title'] ?? ''));
        ?>
        ...
        <div class="mz-text-center mz-group how-it-items mz-grid mz-gap-3 xl:mz-gap-3">
          <div class="mz-relative mz-w-[220px] mz-h-[220px] mz-mx-auto">
            <div class="mz-absolute mz-inset-0 mz-rounded-full mz-bg-brand-secondary mz-transition-colors mz-duration-300 mz-group-hover:mz-bg-brand-accent background-shad"></div>
            ...
            <div class="mz-absolute mz-inset-[14px] mz-w-[200px] mz-left-0 mz-h-[200px] mz-rounded-full mz-overflow-hidden mz-bg-white">
              <?php if ($imgp): ?>
                <img
                  ...
                  <?php if (!empty($imgp['srcset'])): ?>srcset="<?php echo esc_attr($imgp['srcset']); ?>"<?php endif; ?>
            ...
            <div class="mz-absolute background-count mz-top-[-10px] mz-left-[-10px] mz-w-[50px] mz-h-[50px] mz-bg-brand-accent mz-transition-colors mz-font-heading mz-rounded-full mz-flex mz-items-center mz-justify-center mz-duration-300 mz-group-hover:mz-bg-white">
