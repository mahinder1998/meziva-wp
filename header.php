...
<head>
  ...
  <style>
    ...
    /* Logo sizing */
    .logo-svg-wrapper svg{ width:130px; }
    @media(min-width:768px){ .logo-svg-wrapper svg{ width:160px; } }
    ...
    /* Hover bridge (prevents close when moving cursor down) */
    ...
    .mz-dropdown-panel a.menu-link{
      display:block;
      padding:10px 12px;
      border-radius:10px;
      font-size:14px;
    }
    .mz-dropdown-panel a.menu-link:hover{
    ...
    /* =========================
       ...
       ========================= */
    .meziva-mobile-menu{ width:100%; }
    .meziva-mobile-menu > li{ padding: 6px 0; }
...
<header data-meziva-header class="mz-backdrop-blur">
  <div class="mz-max-w-[1290px] mz-mx-auto mz-px-4 xl:mz-px-0">
    <div class="mz-h-16 md:mz-h-20 mz-grid mz-grid-cols-12 mz-items-center mz-transition-all mz-duration-300">
      ...
      <!-- DESKTOP MENU -->
      <nav class="mz-hidden md:mz-flex md:mz-col-span-6 mz-justify-center" aria-label="Primary navigation" data-mz-desktop-nav>
        <?php
...
(function(){
  ...
  if(openBtn && closeBtn && overlay && drawer){
    ...
    // close drawer on normal link click (not accordion toggles)
    drawer.addEventListener('click', function(e){
      const a = e.target.closest('a');
      if(!a) return;
      const href = (a.getAttribute('href') || '').trim();
      if(!href || href === '#') return;
      // if it has nested menu, mobile handler will prevent default. so safe to close here:
      closeMenu();
    });
  }
  ...
  /**
   * =========================
   * Desktop dropdown open/close (optimized)
   * Prevent accidental close while moving down
   * =========================
   */
  ...
  if (desktopNav) {
    ...
    items.forEach(li => {
      ...
      const open = () => {
        if (t) clearTimeout(t);
        li.classList.add('mz-dd-open');
      };

      const close = () => {
        if (t) clearTimeout(t);
        t = setTimeout(() => li.classList.remove('mz-dd-open'), 140);
      };
  ...
  /**
   * =========================
   * Mega Menu: switch products on category hover
   * =========================
   */
  ...
  /**
   * =========================
   * Mobile smooth accordion (maxHeight animation)
   * =========================
   */
...
if (mobileNav) {
  mobileNav.addEventListener('click', function (e) {
    ...
    // ✅ more robust: find submenu wrapper anywhere inside this li
    const wrap = li.querySelector('.mz-mobile-subwrap');
    if (!wrap) return;

    // ✅ if this <a> has children, treat as toggle (don’t navigate)
    e.preventDefault();

    const isOpen = li.classList.toggle('is-open');

    // close other open siblings (optional but cleaner UX)
    const siblings = Array.from(li.parentElement?.children || []).filter(x => x !== li);
    siblings.forEach(sib => {
      if (sib.classList?.contains('is-open')) {
        sib.classList.remove('is-open');
        const w = sib.querySelector('.mz-mobile-subwrap');
        if (w) w.style.maxHeight = '0px';
        const plus = sib.querySelector('a [data-mz-plus]');
    ...
    if (isOpen) {
      ...
      const plus = link.querySelector('[data-mz-plus]');
      if (plus) plus.textContent = '−';
    } else {
  ...
  // ✅ init heights (if any open by default)
  mobileNav.querySelectorAll('li.is-open .mz-mobile-subwrap').forEach(w => {
